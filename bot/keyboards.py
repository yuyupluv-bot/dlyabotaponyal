...
def dispatcher_menu(show_role_switch: bool = False) -> str:
    rows = [
        ...
        [_btn("📊 Мои сведения", WHITE, {"cmd": "disp_income"})],
    ]
    if show_role_switch:
        rows.append(_role_switch_row())
    return keyboard(rows)
...
def dispatcher_info_menu() -> str:
    """Menu of the dispatcher «My details» screen."""
    return keyboard([
        [_btn("💳 Изменить реквизиты", BLUE, {"cmd": "disp_payment"})],
        [_btn("⬅️ Назад в главное меню", WHITE, {"cmd": "start"})],
    ])
