...
# VK error 911: "label should be not more than 40 letters length". Long
# labels come from user data (names, addresses), so they are trimmed here
# instead of failing the delivery of a whole message.
VK_MAX_LABEL_LENGTH = 40


def fit_label(label: Any) -> str:
    text = str(label if label is not None else "")
    if len(text) <= VK_MAX_LABEL_LENGTH:
        return text
    return text[:VK_MAX_LABEL_LENGTH - 1].rstrip() + "\u2026"


def _link_btn(label: str, link: str) -> dict:
    return {"action": {"type": "open_link", "label": fit_label(label), "link": link}}


def _btn(label: str, color: str = WHITE, payload: dict | None = None) -> dict:
    action: dict[str, Any] = {"type": "text", "label": fit_label(label)}
...
def keyboard(rows: list[list[dict]], one_time: bool = False, inline: bool = False) -> str:
    ...
    for row_index, row in enumerate(normalized):
        for button_index, button in enumerate(row):
            ...
            if "label" in action:
                action["label"] = fit_label(action["label"])
...
def passenger_menu(
    ...
    if has_booking:
        ...
        booking_button = _btn(
            labels.get("btn_booking", "📅 Бронь дальние поездки"),
            BLUE,
            {"cmd": "booking_start"},
        )
...
def free_drivers_entry_keyboard() -> str:
    """Direct driver selection is intentionally separate from normal ordering."""
    return keyboard([
        [_btn("👤 Выбор водителя", BLUE, {"cmd": "driver_select_list", "page": 1})],
        [_btn("⬅️ Вернуться в главное меню", WHITE, {"cmd": "start"})],
    ])


def free_driver_choices_keyboard(drivers: list, page: int, total_pages: int) -> str:
    """Show only the driver's name and rating; never expose car details here."""
    rows = []
    for driver in drivers:
        name = driver.full_name or ("id" + str(driver.vk_id))
        count = int(driver.rating_count or 0)
        rating = f"⭐ {driver.rating:.1f}" if count else "⭐ —"
        rows.append([_btn(f"{name[:30].rstrip()} — {rating}", BLUE, {
            "cmd": "driver_select", "driver_id": driver.id,
        })])
    nav = []
    if page > 1:
        nav.append(_btn("⬅️", WHITE, {"cmd": "driver_select_list", "page": page - 1}))
    if page < total_pages:
        nav.append(_btn("➡️", WHITE, {"cmd": "driver_select_list", "page": page + 1}))
    if nav:
        rows.append(nav)
    rows.append([_btn("🔄 Обновить список", WHITE, {
        "cmd": "driver_select_list", "page": page,
    })])
    rows.append([_btn("⬅️ Назад", WHITE, {"cmd": "drivers"})])
    rows.append([_btn("🏠 Главное меню", WHITE, {"cmd": "start"})])
    return keyboard(rows)


def direct_driver_text_keyboard() -> str:
    return keyboard([
        [_btn("❌ Отменить выбор", RED, {"cmd": "direct_driver_cancel"})],
        [_btn("🏠 Главное меню", WHITE, {"cmd": "start"})],
    ])


def direct_driver_unavailable_keyboard() -> str:
    return keyboard([
        [_btn("👤 Выбрать другого водителя", BLUE, {
            "cmd": "driver_select_list", "page": 1,
        })],
        [_btn("🚕 Заказать авто", GREEN, {"cmd": "new_order"})],
        [_btn("🏠 Главное меню", WHITE, {"cmd": "start"})],
    ])
...
def extras_keyboard(selection) -> str:
    ...
    for svc in extra_services.SERVICES:
        mark = "✅ " if _is_on(selection, svc["key"]) else "⬜ "
...
def review_comment_keyboard(review_id: int) -> str:
    return keyboard(
        [
            [_btn("Написать комментарий", BLUE, {"cmd": "review_comment_add"})],
            [_btn("Изменить оценку", WHITE, {
                "cmd": "review_rating_change", "review_id": review_id,
            })],
...
def review_rating_keyboard(review_id: int) -> str:
    """Replace an already saved passenger-to-driver rating."""
    rows = [[_btn("⭐" * stars, WHITE, {
        "cmd": "review_rating_set", "review_id": review_id, "stars": stars,
...
def rating_keyboard(order_id: int) -> str:
    ...
    rows.append([_btn("Пропустить", WHITE, {"cmd": "skip_rate", "order_id": order_id})])
    return keyboard(rows, inline=True)
...
def passenger_rating_keyboard(order_id: int) -> str:
    ...
    rows = [[_btn("⭐" * i, WHITE, {"cmd": "rate_passenger", "order_id": order_id, "stars": i})]
            for i in range(5, 0, -1)]
    rows.append([_btn("Пропустить", WHITE, {"cmd": "skip_rate_passenger", "order_id": order_id})])
    return keyboard(rows, inline=True)
...
def booking_take_keyboard(booking_id: int) -> str:
    # Cancellation is automatic after 45 minutes; drivers only see the claim action.
    return keyboard([
...
def driver_ride_keyboard(
    ...
    if stage == "assigned":
        ...
        seated_label = "📦 Забрал заказ" if partner_delivery else "🧍 Пассажир сел"
        rows.append([_btn(seated_label, GREEN, {"cmd": "seated"})])
...
def route_parallel_offer_keyboard(order_id: int) -> str:
    """Direct one-minute offer for a driver travelling to the pickup city."""
    # Transient offer controls must be inline. A regular keyboard replaces the
    # driver's persistent active-ride menu and can remove it later when the
    # tracked offer message is deleted after a refusal.
    return keyboard([
        [_btn("✅ Взять параллельную заявку", GREEN,
              {"cmd": "route_parallel_take", "order_id": order_id})],
        [_btn("❌ Отказаться", RED,
...
def dispatcher_menu(show_role_switch: bool = False) -> str:
    rows = [
        ...
        [_btn("📤 Отправить отчет водителям", BLUE, {"cmd": "disp_driver_report"})],
    ]
    if show_role_switch:
        rows.append(_role_switch_row())
    return keyboard(rows)
...
def dispatcher_booking_menu() -> str:
    return keyboard([
        [_btn("📅 Бронь дальние поездки", GREEN, {"cmd": "disp_booking_new"})],
        [_btn("🗓 Мои брони", BLUE, {"cmd": "disp_bookings"})],
...
def driver_active_cancel_keyboard() -> str:
    return keyboard([
        ...
        [_btn("Клиент захотел отменить", RED, {"cmd":"driver_cancel_customer"})],
...
def chat_take_keyboard(order_id: int) -> str:
    # Orders, deliveries and far rides expire automatically after 45 minutes.
    return keyboard([
