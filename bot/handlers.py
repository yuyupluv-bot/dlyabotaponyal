...
def active_order_for(session: Session, user: User, as_driver: bool = False) -> Order | None:
    ...
    if not as_driver:
        return q.filter(Order.passenger_id == user.id).order_by(Order.created_at.desc()).first()

    # A reserved parallel order must not replace the ride currently shown in
    # the driver's active-order menu.
    order = (
        q.filter(Order.driver_id == user.id, Order.status != "parallel_assigned")
        .order_by(Order.created_at.desc())
        .first()
    )
    if order is None:
        # Compatibility repair for a parallel request promoted by an older
        # process: some rows kept only parallel_driver_id after their status
        # had already changed to an ordinary active stage. Without this repair
        # the finish-price input cannot find the ride and appears to do nothing.
        order = (
            session.query(Order)
            .filter(
                Order.parallel_driver_id == user.id,
    ...
    if order is not None and order.parallel_driver_id == user.id:
        if order.arrival_eta is None and order.parallel_eta is not None:
            order.arrival_eta = max(0, int(order.parallel_eta))
...
# The selected UI role may change while a driver still has a durable input
# state (for example, a person has both Driver and Dispatcher roles).  These
# states always belong to the driver's active workflow and must never be sent
# to the dispatcher handler just because the menu role changed in the meantime.
_DRIVER_OWNED_INPUT_STATES = {
...
def _chat_claim_reply(session: Session, driver: User, text: str) -> None:
    """Answer a driver inside the requests chat.

    A claim that cannot be completed used to produce only a private message,
    so in the chat it looked like the button did nothing and the request kept
    hanging without an owner. Every refusal is now visible where it happened.
    """
    name = driver.full_name or ("id" + str(driver.vk_id))
    order_service.send_fallback_chat_notice(session, f"[id{driver.vk_id}|{name}], {text}")
...
def _release_stale_offer_for_active_ride(
    session: Session,
    driver: User,
    pending_offer: Order,
    current_order: Order | None = None,
) -> bool:
    """Re-offer an ordinary card that raced with the driver's active ride.

    A driver may never have both an active trip and an unanswered ordinary
    offer.  Older workers and a rare cross-transaction race could leave that
    combination behind.  Besides being incorrect, the offer lock then
    overwrote states such as ``D_FINISH_PRICE`` and made the entered fare look
    ignored.  Preserve the active trip's state, repair the queue status, and
    hand the stale card to another eligible driver.
    """
    current_order = current_order or active_order_for(session, driver, as_driver=True)
    if current_order is None or current_order.id == pending_offer.id:
        return False

    offer = (
        session.query(Order)
        .filter(Order.id == pending_offer.id)
        .with_for_update()
        .one_or_none()
    )
    if (
        offer is None
        or offer.status != "searching"
        or offer.offered_driver_id != driver.id
        or offer.driver_id is not None
    ):
        return False

    timers.cancel("accept", offer.id)
    order_service.finalize_offer_message(
        session,
        offer,
        "Заявка передана следующему свободному водителю.",
    )
    offer.offered_driver_id = None
    offer.status = "created"
    # Repair a legacy/raced queue row before selecting the next recipient.
    queue_service.mark_assigned(session, driver)
    audit.record(
        session,
        "stale_offer_released",
        f"offer={offer.id} active={current_order.id} driver={driver.id}",
    )
    order_service.offer_to_next_driver(
        session,
        offer,
        temporary_exclude_driver_ids={driver.id},
    )
    return True
...
#  Entry point                                                                 #
# --------------------------------------------------------------------------- #
HANDLER_ERROR_NOTICE = (
    "\u26a0\ufe0f Не получилось обработать действие. Попробуйте ещё раз или напишите «меню»."
)


def handle_message(session: Session, event) -> None:
    """Wrapper: a crashing handler must never answer a person with silence.

    The exception is re-raised so the transaction is rolled back and no
    half-written order is committed, but the person always gets a reply.
    """
    try:
        return _handle_message_impl(session, event)
    except Exception as exc:  # noqa: BLE001
        log.exception("handle_message failed: %s", exc)
        try:
            message = getattr(getattr(event, "obj", None), "message", None) or {}
            vk_id = int(message.get("from_id") or 0)
            peer_id = int(message.get("peer_id") or vk_id)
            if vk_id > 0 and peer_id == vk_id:
                vk.send_message(vk_id, HANDLER_ERROR_NOTICE)
        except Exception:  # noqa: BLE001
            log.exception("Failed to notify the user about a handler error")
        raise


def _handle_message_impl(session: Session, event) -> None:
    ...
    if vk_id in config.ADMIN_VK_IDS and not user.has_role(ROLE_ADMIN):
        ...
        if user.rating_count == 0 and user.role == ROLE_PASSENGER:
            user.role = ROLE_ADMIN
    cmd = payload.get("cmd")
    # Some VK clients occasionally send the regular-keyboard label without
    # its payload.  The parallel-list back button then looks pressed in chat
    # but the bot receives only plain text.  Recover this one safe navigation
    # action from its exact label instead of silently forwarding it to chat.
    if not cmd and user.has_role(ROLE_DRIVER):
        plain_button_text = re.sub(r"^[^\wа-яё]+", "", text.casefold()).strip()
        if plain_button_text in (
            "вернуться к активной заявке",
            "вернуться к заявке",
        ):
            payload = {"cmd": "parallel_back"}
            cmd = "parallel_back"
    ...
    if is_conversation:
        ...
        allowed_conversation_cmds = ("chat_take", "booking_take")
        if cmd in allowed_conversation_cmds and user.has_role(ROLE_DRIVER):
            pending_offer = offered_order_for(session, user)
            if pending_offer and not queue_service.offer_lock_is_live(session, user):
                # A dead reservation must never lock a driver out of the chat.
                pending_offer.offered_driver_id = None
                session.flush()
                log.warning(
                    "Released stale offer lock order=%s driver=%s before chat claim",
                    pending_offer.id,
                    user.id,
                )
                pending_offer = None
            # A claim in the requests chat is never blocked by a pending
            # offer: driver_take_from_chat hands that offer to the next
            # driver. Only the booking button keeps the old lock.
            if pending_offer and cmd != "chat_take":
                _chat_claim_reply(
                    session,
                    user,
                    f"сначала ответьте на заявку #{pending_offer.id} в личных сообщениях бота.",
                )
        ...
        if cmd == "chat_take":
            if not user.has_role(ROLE_DRIVER):
                _chat_claim_reply(session, user, "взять заявку может только водитель.")
    ...
    pending_offer = None
    if user.has_role(ROLE_DRIVER):
        pending_offer = offered_order_for(session, user)
        current_driver_order = active_order_for(session, user, as_driver=True)
        if pending_offer and current_driver_order:
            if _release_stale_offer_for_active_ride(
                session, user, pending_offer, current_driver_order
            ):
                pending_offer = None

        # A durable driver input state always wins over an ordinary offer.
        # In particular, a fare typed after «🏁 Завершить» must not be turned
        # into D_OFFER by a stale offer card.
        if state in _DRIVER_OWNED_INPUT_STATES:
            return handle_driver(session, user, state, text, payload, attachments)
    ...
    if user.role == ROLE_DRIVER and pending_offer:
        if cmd not in _DRIVER_OFFER_ALLOWED_CMDS:
            return _driver_offer_lock_notice(session, user, pending_offer)

    # Leaving the 90-second direct-selection input must release the driver;
    # otherwise a plain «Меню» would strand an invisible reservation.
    is_start_command = (
        text.lower() in ("начать", "start", "/start", "меню") or cmd == "start"
    )
    if state == States.P_DIRECT_TEXT and (is_start_command or cmd == "change_role"):
        cancel_direct_driver_selection(
            session, user, notify_passenger=False, notify_driver=True
        )
...
def do_role_set(session: Session, user: User, role: str | None) -> None:
    ...
    if user.role == ROLE_DRIVER and role != ROLE_DRIVER and user.driver_status != "offline":
        # Use the same safe departure path so a selected-driver offer is
        # cancelled instead of leaking into the ordinary FIFO queue.
        driver_go_offline(session, user)
    user.role = role
    show_main_menu(session, user)
...
DIRECT_DRIVER_TEXT_TIMEOUT = 90
DIRECT_DRIVER_PAGE_SIZE = 6


def _show_driver_status_overview(session: Session, user: User) -> None:
    """Read-only status list retained for passengers who already have a flow."""
    ...
    considering = [driver for driver in drivers if statuses.get(driver.id) == "considering"]
    busy = [driver for driver in drivers if statuses.get(driver.id) == "busy"]
    away = [driver for driver in drivers if statuses.get(driver.id) == "away"]
    visible = free + considering + busy + away
    ...
    lines = [
        "🚗 Водители сейчас:",
        f"Свободны: {len(free)}",
        f"Рассматривают заявку: {len(considering)}",
        f"На заявке: {len(busy)}",
        f"Отлучились: {len(away)}",
        f"Всего с учётом отлучившихся: {len(visible)}",
        "",
    ]
    for driver in visible:
        name = driver.full_name or "Водитель"
        driver_status = statuses.get(driver.id)
        destination = None
        if driver_status == "online":
            status = "Свободен"
        elif driver_status == "considering":
            status = "Рассматривает заявку"
        elif driver_status == "away":
            status = "Отлучился"
        else:
        ...
        if driver_status == "busy" and destination in ("Пашия", "Кусья"):
            lines.append(f"• {name} — {status}")
        else:
            location = driver.current_line or "Локация не указана"
            lines.append(f"• {name} — {status} — {location}")
    if not visible:
        lines.append("Свободных, занятых и отлучившихся водителей сейчас нет.")
...
def show_free_drivers(session: Session, user: User) -> None:
    """Open direct selection only from the idle passenger main menu."""
    state = get_state(session, user.vk_id).state
    if active_order_for(session, user) or state not in (
        States.MAIN_MENU, States.P_DRIVER_SELECT,
    ):
        return _show_driver_status_overview(session, user)
    set_state(session, user.vk_id, States.P_DRIVER_SELECT, {}, merge=False)
    vk.send_message(
        user.vk_id,
        "🚗 Свободные водители\n\nВы можете посмотреть список и выбрать конкретного водителя.",
        keyboard=kb.free_drivers_entry_keyboard(),
    )


def _direct_driver_candidates(session: Session) -> list[User]:
    """Only genuinely free drivers may appear in the direct-selection menu."""
    return [
        driver for driver in queue_service.free_drivers(session)
        if driver.is_on_line
        and driver.driver_status == "online"
        and lines.has_complete_car(driver)
    ]


def show_direct_driver_choices(session: Session, user: User, page=1) -> None:
    ...
    try:
        ...
        page = 1
    drivers = _direct_driver_candidates(session)
    total_pages = max(1, math.ceil(len(drivers) / DIRECT_DRIVER_PAGE_SIZE))
    page = min(page, total_pages)
    start = (page - 1) * DIRECT_DRIVER_PAGE_SIZE
    visible = drivers[start:start + DIRECT_DRIVER_PAGE_SIZE]
    if visible:
        text_lines = [f"👤 Выбор водителя — страница {page}/{total_pages}:", ""]
        for driver in visible:
            name = driver.full_name or ("id" + str(driver.vk_id))
            text_lines.append(f"• {name} — {format_rating(driver)}")
        text_lines.extend(("", "Нажмите на имя нужного водителя."))
        text = "\n".join(text_lines)
    else:
        text = "Сейчас нет водителей со статусом «Свободен». Обновите список немного позже."
    set_state(session, user.vk_id, States.P_DRIVER_SELECT, {}, merge=False)
    vk.send_message(
        user.vk_id,
        text,
        keyboard=kb.free_driver_choices_keyboard(visible, page, total_pages),
    )


def _release_direct_selection(session: Session, selection: DriverSelection) -> None:
    """Release exactly this pre-order hold without touching a later real offer."""
    timers.cancel("direct_selection", selection.id)
    entry = (
        session.query(DriverQueue)
        .filter(DriverQueue.driver_id == selection.driver_id)
        .with_for_update()
        .one_or_none()
    )
    live_offer = session.query(Order.id).filter(
        Order.offered_driver_id == selection.driver_id,
        Order.status == "searching",
    ).first()
    if entry and entry.status == "offered" and not live_offer:
        entry.status = "waiting"
    session.delete(selection)


def cancel_direct_driver_selection(
    session: Session,
    user: User,
    *,
    notify_passenger: bool = True,
    notify_driver: bool = True,
) -> bool:
    selection = (
        session.query(DriverSelection)
        .filter(DriverSelection.passenger_id == user.id)
        .with_for_update()
        .one_or_none()
    )
    if not selection:
        return False
    driver = session.get(User, selection.driver_id)
    _release_direct_selection(session, selection)
    reset(session, user.vk_id, States.MAIN_MENU)
    if notify_passenger:
        vk.send_message(
            user.vk_id,
            "Выбор водителя отменён.",
            keyboard=kb.passenger_menu(can_switch_role(user), _passenger_labels(session)),
        )
    if notify_driver and driver:
        vk.send_message(driver.vk_id, "Пассажир отменил выбор до отправки заявки.")
    passenger_queue.try_promote(session)
    return True


def cancel_direct_selection_for_driver(session: Session, driver: User) -> bool:
    """Cancel the pre-text hold if the selected driver leaves or changes role."""
    selection = (
        session.query(DriverSelection)
        .filter(DriverSelection.driver_id == driver.id)
        .with_for_update()
        .one_or_none()
    )
    if not selection:
        return False
    passenger = session.get(User, selection.passenger_id)
    selection_id = selection.id
    _release_direct_selection(session, selection)
    if passenger:
        data = get_data(session, passenger.vk_id)
        if data.get("selection_id") == selection_id:
            reset(session, passenger.vk_id, States.MAIN_MENU)
        vk.send_message(
            passenger.vk_id,
            "Выбранный водитель больше не свободен. Выберите другого водителя.",
            keyboard=kb.direct_driver_unavailable_keyboard(),
        )
    passenger_queue.try_promote(session)
    return True


def select_direct_driver(session: Session, user: User, driver_id) -> None:
    if active_order_for(session, user):
        return _menu_lock_notice(session, user)
    ...
    try:
        ...
        return show_direct_driver_choices(session, user, 1)
    ...
    if old:
        _release_direct_selection(session, old)
        session.flush()

    entry = (
        session.query(DriverQueue)
        .filter(DriverQueue.driver_id == driver_id)
        .with_for_update()
        .one_or_none()
    )
    driver = session.get(User, driver_id)
    available = bool(
        entry and entry.status == "waiting"
        and driver and driver.has_role(ROLE_DRIVER)
        and driver.is_on_line and driver.driver_status == "online"
        and not driver.is_blocked and lines.has_complete_car(driver)
        and not active_order_for(session, driver, as_driver=True)
        and not offered_order_for(session, driver)
    )
    if not available:
        vk.send_message(user.vk_id, "Этот водитель уже не свободен. Список обновлён.")
        return show_direct_driver_choices(session, user, 1)
    ...
    selection = DriverSelection(
        ...
        driver_id=driver.id,
        expires_at=time_utils.now() + dt.timedelta(seconds=DIRECT_DRIVER_TEXT_TIMEOUT),
    )
    ...
    set_state(
        session,
        user.vk_id,
        States.P_DIRECT_TEXT,
        {"selection_id": selection.id, "driver_id": driver.id},
        merge=False,
    )
    timers.schedule(
        "direct_selection",
        selection.id,
        DIRECT_DRIVER_TEXT_TIMEOUT,
        lambda: direct_driver_selection_timeout(selection.id),
    )
    name = driver.full_name or ("id" + str(driver.vk_id))
    vk.send_message(
        user.vk_id,
        f"Вы выбрали водителя: {name} — {format_rating(driver)}.\n\n"
        "Напишите текст заявки одним сообщением. На отправку даётся 90 секунд.",
        keyboard=kb.direct_driver_text_keyboard(),
    )
    vk.send_message(
        driver.vk_id,
        "Пассажир выбрал вас и сейчас пишет персональную заявку. "
        "До её отправки оставайтесь на линии; ожидание займёт не больше 90 секунд.",
    )


def direct_driver_selection_timeout(selection_id: int) -> None:
    from common.database import session_scope

    with session_scope() as session:
        selection = (
            session.query(DriverSelection)
            .filter(DriverSelection.id == selection_id)
            .with_for_update()
            .one_or_none()
        )
        if not selection:
        ...
        if passenger:
            ...
            vk.send_message(
                passenger.vk_id,
                "90 секунд истекли. Выбранный водитель снова доступен.",
                keyboard=kb.direct_driver_unavailable_keyboard(),
            )
        if driver:
            vk.send_message(driver.vk_id, "Пассажир не отправил заявку за 90 секунд. Вы снова свободны.")
        passenger_queue.try_promote(session)


def direct_driver_submit_text(session: Session, user: User, text: str) -> None:
    """Turn the 90-second hold into a normal order offered only to its driver."""
    raw = " ".join((text or "").split())
    if _is_not_order_text(session, raw):
        return vk.send_message(
            user.vk_id,
            "Напишите полный текст заявки: откуда и куда нужно ехать.",
            keyboard=kb.direct_driver_text_keyboard(),
        )
    ...
    selection = (
        session.query(DriverSelection)
        .filter(
            DriverSelection.id == selection_id,
            DriverSelection.passenger_id == user.id,
        )
        .with_for_update()
        .one_or_none()
        if selection_id else None
    )
    if not selection or selection.expires_at <= time_utils.now():
        if selection:
            _release_direct_selection(session, selection)
        reset(session, user.vk_id, States.P_DRIVER_SELECT)
        vk.send_message(
            user.vk_id,
            "Время выбора истекло или водитель уже недоступен.",
            keyboard=kb.direct_driver_unavailable_keyboard(),
        )
        return
    entry = (
    ...
    driver = session.get(User, selection.driver_id)
    if (
        not entry or entry.status != "offered" or not driver
        or not driver.is_on_line or driver.driver_status != "online"
        or active_order_for(session, driver, as_driver=True)
        or offered_order_for(session, driver)
    ):
        _release_direct_selection(session, selection)
        reset(session, user.vk_id, States.P_DRIVER_SELECT)
        vk.send_message(
            user.vk_id,
            "Выбранный водитель уже недоступен. Выберите другого.",
            keyboard=kb.direct_driver_unavailable_keyboard(),
        )
        return
    ...
    draft = {
        ...
        "driver_selection_id": selection.id,
    }
    set_state(session, user.vk_id, States.P_ADDR, {"draft": draft}, merge=False)
    return order_set_addresses(session, user, raw)
...
def handle_passenger(session, user, state, text, payload, attachments):
    ...
    if cmd == "cancel_flow":
        if state == States.P_DIRECT_TEXT:
            return cancel_direct_driver_selection(session, user)
        reset(session, user.vk_id, States.MAIN_MENU)
        return show_main_menu(session, user)
    if cmd == "direct_driver_cancel":
        if cancel_direct_driver_selection(session, user):
            return
        return show_main_menu(session, user)
    if cmd == "driver_select_list":
        if state not in (States.MAIN_MENU, States.P_DRIVER_SELECT):
            return show_main_menu(session, user)
        return show_direct_driver_choices(session, user, payload.get("page", 1))
    if cmd == "driver_select":
        if state != States.P_DRIVER_SELECT:
            return show_main_menu(session, user)
        return select_direct_driver(session, user, payload.get("driver_id"))
    if cmd == "rules":
    ...
    if cmd == "departure_cancel":
        return passenger_departure_response(session, user, cancel=True)
    ...
    if cmd == "review_rating_change" and state == States.P_REVIEW_TEXT:
        return passenger_review_rating_change(session, user, payload.get("review_id"))
    if cmd == "review_rating_set" and state == States.P_REVIEW_TEXT:
        return passenger_review_rating_set(
            session, user, payload.get("review_id"), payload.get("stars")
        )
    if cmd == "going_out":
    ...
    if cmd == "skip_rate":
        return passenger_skip_rating(session, user, payload.get("order_id"))
    ...
    if state == States.P_DIRECT_TEXT:
        return direct_driver_submit_text(session, user, text)
...
# Short courtesy replies must open the menu, not create a request.
TEXT_ORDER_STOP_WORDS = frozenset({
    "ок", "окей", "ok", "okay", "хорошо", "да", "нет", "ага", "угу",
    "спс", "спасибо", "пасиб", "благодарю", "привет", "здравствуйте",
    "здравствуй", "добрый день", "доброе утро", "добрый вечер",
    "доброй ночи", "пока", "до свидания", "алло", "ало", "тут", "здесь",
    "жду", "ждем", "ждём", "понял", "поняла", "ясно", "хм", "где",
    "когда", "сколько", "а", "эй", "test", "тест", "проверка",
    # реакция на цену
    "дорого", "дорога", "дороговато", "это дорого", "очень дорого",
    "дорого так", "что так дорого", "почему так дорого",
    "дёшевле можно", "дешевле можно",
    # вопросы про наличие машин
    "есть машинки", "есть машинка", "есть машины", "есть машина",
    "машинки есть", "машинка есть", "машины есть", "машина есть",
    "есть свободные", "есть свободный", "есть свободная",
    "есть свободные машины", "есть свободные машинки",
    "есть свободные водители", "свободные есть",
    "есть водители", "водители есть", "есть кто", "есть кто нибудь",
    "есть кто-нибудь", "кто свободен", "кто нибудь есть",
    "кто-нибудь есть", "машинки", "машина", "свободные",
    # вопросы про цену без адреса
    "сколько стоит", "сколько будет", "почем", "почём", "цена",
    "какая цена", "цены", "тариф", "тарифы",
})


def _is_not_order_text(session: Session, raw: str) -> bool:
    """True when a plain message is chatter rather than a ride request."""
    normalized = (raw or "").strip().strip("!?.,;:").casefold()
    if not normalized:
        return True
    if normalized in TEXT_ORDER_STOP_WORDS:
        return True
    if normalized in _menu_text_commands(session):
        return True
    if not any(char.isalnum() for char in normalized):
        return True
    return len(normalized) < TEXT_ORDER_MIN_LENGTH


def _menu_text_commands(session: Session) -> set:
    """Texts that must open the menu instead of creating an order."""
    values = set(_passenger_labels(session).values())
    values.update({
        "начать", "старт", "start", "/start", "menu", "меню",
        "главное меню", "назад", "стоп", "stop", "отмена",
    })
    return {str(v).strip().casefold() for v in values if v}


def passenger_text_order(session: Session, user: User, text: str) -> None:
    """Create an order from a plain passenger text message.

    A passenger does not have to press «Заказать авто» first: any free-form
    message outside of an active order/flow is treated as the order text.
    """
    raw = " ".join((text or "").split())
    if _is_not_order_text(session, raw):
        return show_main_menu(session, user)
    ...
    if active_order_for(session, user):
        ...
        return
    set_state(session, user.vk_id, States.P_ADDR, {"draft": {"order_type": "regular"}}, merge=False)
    return order_set_addresses(session, user, raw)
...
def order_set_addresses(session: Session, user: User, text: str) -> None:
    ...
    # «Красный бархат»/«Галактика» + слово «доставка» → доставка от партнёров.
    # Название партнёра без слова «доставка» остаётся обычной поездкой.
    if delivery_service.is_partner_delivery_text(raw):
        draft["order_type"] = delivery_service.PARTNER_DELIVERY_TYPE
    elif delivery_service.is_delivery_text(raw):
...
def create_passenger_order(session: Session, user: User, order_type: str) -> None:
    ...
    if not draft.get("address_from") or not draft.get("address_to"):
        return start_new_order(session, user)
    requested_driver_id = draft.get("requested_driver_id")
    selection_id = draft.get("driver_selection_id")
    direct_selection = None
    selected_driver = None
    if requested_driver_id or selection_id:
        try:
            requested_driver_id = int(requested_driver_id)
            selection_id = int(selection_id)
        except (TypeError, ValueError):
            requested_driver_id = selection_id = None
        direct_selection = (
            session.query(DriverSelection)
            .filter(
                DriverSelection.id == selection_id,
                DriverSelection.passenger_id == user.id,
                DriverSelection.driver_id == requested_driver_id,
            )
            .with_for_update()
            .one_or_none()
            if selection_id and requested_driver_id else None
        )
        selected_driver = session.get(User, requested_driver_id) if direct_selection else None
        entry = (
            session.query(DriverQueue)
            .filter(DriverQueue.driver_id == requested_driver_id)
            .with_for_update()
            .one_or_none()
            if selected_driver else None
        )
        valid_selection = bool(
            direct_selection and direct_selection.expires_at > time_utils.now()
            and entry and entry.status == "offered"
            and selected_driver.is_on_line
            and selected_driver.driver_status == "online"
            and not active_order_for(session, selected_driver, as_driver=True)
            and not offered_order_for(session, selected_driver)
        )
        if not valid_selection:
            if direct_selection:
                _release_direct_selection(session, direct_selection)
            reset(session, user.vk_id, States.P_DRIVER_SELECT)
            return vk.send_message(
                user.vk_id,
                "Выбранный водитель уже недоступен. Выберите другого.",
    ...
    normalized_type = (
        order_type
        if order_type in ("delivery", delivery_service.PARTNER_DELIVERY_TYPE)
        else "regular"
    )
    extras_selection = data.get("extras", []) if normalized_type == "regular" else []
    order = Order(
        passenger_id=user.id,
        requested_driver_id=requested_driver_id if selected_driver else None,
        city_id=draft.get("city_id"),
        ...
        extra_services=extra_services.to_json(extras_selection),
        night_surcharge=(normalized_type == "regular" and night_tariff.is_night(session)),
    )
    session.add(order)
    session.flush()
    if direct_selection:
        timers.cancel("direct_selection", direct_selection.id)
        session.delete(direct_selection)
        session.flush()
    audit.record(session, "order_created", f"order={order.id} type={order.order_type} extras={extras_selection}")
    ...
    if selected_driver:
        if order_service.offer_to_selected_driver(session, order, selected_driver):
            return
        order.status = "cancelled"
        order.cancelled_at = time_utils.now()
        order.cancelled_by = "system"
        queue_service.release_offer(session, selected_driver)
        reset(session, user.vk_id, States.MAIN_MENU)
        vk.send_message(
            user.vk_id,
            "Выбранный водитель уже недоступен. Заявка не передавалась в общую очередь.",
            keyboard=kb.direct_driver_unavailable_keyboard(),
        )
        passenger_queue.try_promote(session)
        return
...
def passenger_order_status(session: Session, user: User) -> None:
    ...
    if order.parallel_eta:
        text += f" Водитель завершает текущую поездку и будет у вас ориентировочно через {order.parallel_eta} мин."
...
def passenger_departure_response(session: Session, user: User, cancel: bool) -> None:
    ...
    if prompt_id:
        from . import outbox_service
        removed = outbox_service.finalize_tracked_message(
            session, prompt_id, "✅ Вы ответили: Да, жду."
        )
    order.departure_prompt_outbox_id = None
    # Controls for the active ride remain available in a separate message;
    # the old Да/Нет choice itself has no buttons anymore.
...
#  Rating + reviews                                                            #
# --------------------------------------------------------------------------- #
def _finish_rating_prompt(session: Session, order: Order, driver_side: bool, note: str) -> None:
    """Remove stars/skip from the exact rating card after one answer."""
    field = "driver_rating_prompt_outbox_id" if driver_side else "passenger_rating_prompt_outbox_id"
    outbox_id = getattr(order, field, None)
    if outbox_id:
        from . import outbox_service
        outbox_service.finalize_tracked_message(session, outbox_id, note)
        setattr(order, field, None)


def passenger_skip_rating(session: Session, user: User, order_id: int | None) -> None:
    order = session.get(Order, int(order_id)) if order_id else (
        session.query(Order)
        .filter(Order.passenger_id == user.id, Order.passenger_rating_prompt_outbox_id.isnot(None))
        .order_by(Order.completed_at.desc())
        .first()
    )
    if order and order.passenger_id == user.id:
        _finish_rating_prompt(session, order, False, "Оценка пропущена.")
    vk.send_message(user.vk_id, "Спасибо! Ждём вас снова.", keyboard=kb.passenger_menu(can_switch_role(user)))
    reset(session, user.vk_id, States.MAIN_MENU)


def driver_skip_passenger_rating(session: Session, user: User, order_id: int | None) -> None:
    order = session.get(Order, int(order_id)) if order_id else (
        session.query(Order)
        .filter(Order.driver_id == user.id, Order.driver_rating_prompt_outbox_id.isnot(None))
        .order_by(Order.completed_at.desc())
        .first()
    )
    if order and order.driver_id == user.id:
        _finish_rating_prompt(session, order, True, "Оценка пропущена.")
    vk.send_message(user.vk_id, "Оценка пропущена. Выберите действие с линией в меню ниже.")
...
def save_rating(session: Session, user: User, order_id: int, stars: int) -> None:
    ...
    existing = session.query(Review).filter(
        ...
        Review.kind == "passenger_to_driver",
    ).first()
    if existing:
        _finish_rating_prompt(session, order, False, "Оценка уже сохранена.")
        return
    stars = max(1, min(5, int(stars)))
    _finish_rating_prompt(session, order, False, f"✅ Ваша оценка: {stars} из 5.")
    order.rating = stars
    review = Review(
        order_id=order.id,
    ...
    if stars < 5:
        ...
        vk.send_message(
            ...
            keyboard=kb.review_comment_keyboard(review.id),
        )
        return
    vk.send_message(
        user.vk_id,
...
def _passenger_review_for_change(
    session: Session,
    user: User,
    review_id,
) -> Review | None:
    """Return only the active passenger review from the current rating flow."""
    try:
    ...
    data = get_data(session, user.vk_id)
    try:
        active_review_id = int(data.get("review_id") or 0)
    except (TypeError, ValueError):
        return None
    if active_review_id != review_id:
        return None
    review = session.get(Review, review_id)
    if (
        review is None
        or review.passenger_id != user.id
        or review.kind != "passenger_to_driver"
    ):
        return None
    return review


def passenger_review_rating_change(session: Session, user: User, review_id) -> None:
    review = _passenger_review_for_change(session, user, review_id)
    if review is None:
        return show_main_menu(session, user)
    vk.send_message(
        user.vk_id,
        "Выберите новую оценку от 1 до 5 звёзд:",
        keyboard=kb.review_rating_keyboard(review.id),
    )


def passenger_review_rating_set(
    session: Session,
    user: User,
    review_id,
    stars,
    ...
    try:
        ...
        return passenger_review_rating_change(session, user, review.id)

    old_stars = int(review.stars or 0)
    review.stars = stars
    order = session.get(Order, review.order_id) if review.order_id else None
    if order and order.passenger_id == user.id:
        order.rating = stars
    driver = session.get(User, review.driver_id) if review.driver_id else None
    if driver:
        driver.rating_sum = max(0, int(driver.rating_sum or 0) + stars - old_stars)
    ...
    if stars < 5:
        set_state(
            ...
            user.vk_id,
            States.P_REVIEW_TEXT,
            {"review_id": review.id, "order_id": review.order_id},
            merge=False,
        )
        vk.send_message(
            user.vk_id,
            f"✅ Оценка изменена: {stars} из 5. Хотите оставить комментарий?",
            keyboard=kb.review_comment_keyboard(review.id),
        )
        return
    ...
    vk.send_message(
        user.vk_id,
        "✅ Оценка изменена на 5 из 5. Спасибо за оценку! 🙏",
        keyboard=kb.passenger_menu(can_switch_role(user), _passenger_labels(session)),
    )
    reset(session, user.vk_id, States.MAIN_MENU)
...
def _ask_driver_rate_passenger(session: Session, user: User, order: Order) -> None:
    ...
    if delivery_service.is_partner_delivery(order):
        # По доставкам от партнёров оценку ставит только заказчик.
    ...
    if not _is_dispatcher_order(order) and order.passenger_id:
        ...
        order.driver_rating_prompt_outbox_id = vk.send_tracked_message(
            user.vk_id,
...
def driver_rate_passenger(session: Session, user: User, order_id: int, stars: int) -> None:
    ...
    existing = session.query(Review).filter(
        Review.order_id == order.id,
        Review.kind == "driver_to_passenger",
    ).first()
    if existing:
        _finish_rating_prompt(session, order, True, "Оценка уже сохранена.")
        return
    stars = max(1, min(5, int(stars)))
    _finish_rating_prompt(session, order, True, f"✅ Ваша оценка: {stars} из 5.")
    review = Review(
        order_id=order.id,
        driver_id=user.id,
...
def handle_driver(session, user, state, text, payload, attachments):
    ...
    pending_offer = offered_order_for(session, user)
    current_order = active_order_for(session, user, as_driver=True)
    if pending_offer and current_order:
        if _release_stale_offer_for_active_ride(
            session, user, pending_offer, current_order
        ):
            pending_offer = None

    # Plain text while the fare prompt is active completes the ride. Buttons
    # remain usable: a driver may open the price list, inspect parallel orders
    # or cancel the trip before finally entering the amount.
    if state == States.D_FINISH_PRICE and not cmd:
    ...
    if cmd == "skip_rate_passenger":
        return driver_skip_passenger_rating(session, user, payload.get("order_id"))
    ...
    if cmd == "parallel_back":
        return driver_return_to_active(session, user)
    if cmd == "parallel_take":
        from . import parallel_orders
        return parallel_orders.take(session, user, payload.get("order_id"))
    if cmd == "route_parallel_take":
        from . import parallel_orders
        return parallel_orders.take_route_offer(session, user, payload.get("order_id"))
...
def show_who_on_line(session: Session, user: User) -> None:
    ...
    groups = [
        ("Свободны", "online"),
        ("Рассматривают заявку", "considering"),
        ("На заявке", "busy"),
        ("Отлучились", "away"),
        ("Не на линии", "offline"),
    ]
...
def _cancel_direct_offer_for_unavailable_driver(
    session: Session, order: Order, driver: User
) -> None:
    """Close a personal offer when its chosen driver leaves; never requeue it."""
    timers.cancel("accept", order.id)
    order_service.add_decline(session, order, driver.id, "away")
    order_service.clear_current_offer(session, order)
    order.status = "cancelled"
    order.cancelled_at = time_utils.now()
    ...
    if passenger:
        ...
        vk.send_message(
            passenger.vk_id,
            "Выбранный водитель больше не может рассмотреть заявку. "
            "Она не передавалась другим водителям.",
            keyboard=kb.direct_driver_unavailable_keyboard(),
        )


def driver_go_offline(session: Session, user: User) -> None:
    cancel_direct_selection_for_driver(session, user)
    # Immediately pass on an unanswered offer instead of leaving it stuck until
    # the admin-configured timeout.
    data = get_data(session, user.vk_id)
    pending = (
        session.query(Order)
        .filter(Order.id == data.get("order_id"))
        .with_for_update()
        .one_or_none()
        if data.get("order_id") else None
    )
    reassign = bool(pending and pending.status == "searching"
                    and order_service._current_offer(session, pending) == user.id)
    direct_cancelled = bool(reassign and pending.requested_driver_id)
    if direct_cancelled:
        _cancel_direct_offer_for_unavailable_driver(session, pending, user)
    elif reassign:
    ...
    if reassign and not direct_cancelled:
        order_service.clear_current_offer(session, pending)
...
def driver_go_away(session: Session, user: User) -> None:
    ...
    set_state(session, user.vk_id, States.D_MENU)
    if reassign and pending.requested_driver_id:
        _cancel_direct_offer_for_unavailable_driver(session, pending, user)
    elif reassign:
        timers.cancel("accept", pending.id)
        order_service.add_decline(session, pending, user.id, "away")
...
def show_queue(session: Session, user: User, dispatcher_view: bool = False) -> None:
    """Show the canonical driver queue to a driver or dispatcher."""
    active = None if dispatcher_view else active_order_for(session, user, as_driver=True)
    if active:
    ...
    live_statuses = queue_service.actual_driver_statuses(
        session, [item["driver"] for item in entries]
    )
    lines = []
    if dispatcher_view:
        lines.append("📋 Очередь водителей (как в меню водителя):\n")
    elif rank:
    ...
    if not entries:
        ...
        for i, item in enumerate(entries, start=1):
            ...
            live_status = live_statuses.get(d.id, "offline")
            if live_status == "busy" or queue_status == "assigned":
                status_dot, status_text = "🔴", "на заявке"
            elif queue_status == "offered":
                status_dot, status_text = "🔴", "рассматривает заявку"
            elif live_status == "away" or queue_status == "away":
                status_dot, status_text = "🟡", "отлучился"
            elif live_status == "offline":
                status_dot, status_text = "⚪", "не на линии"
            else:
...
def driver_accept(session: Session, user: User, order_id: int) -> None:
    ...
    if not lines.has_complete_car(user):
        ...
        order_service.finalize_offer_message(
            session, order, "🚫 Заявка отменена: данные автомобиля не заполнены."
        )
        order.offered_driver_id = None
        queue_service.leave_queue(session, user)
        user.is_on_line = False
        if order.requested_driver_id:
            order.status = "cancelled"
            order.cancelled_at = time_utils.now()
            order.cancelled_by = "system"
            passenger = session.get(User, order.passenger_id)
            if passenger:
                ...
                vk.send_message(
                    passenger.vk_id,
                    "Выбранный водитель больше недоступен. Персональная заявка отменена.",
                    keyboard=kb.direct_driver_unavailable_keyboard(),
                )
            vk.send_message(
                user.vk_id,
                "🚗 Персональная заявка отменена: сначала полностью заполните «Моя машина».",
                keyboard=kb.driver_menu(on_line=False, show_role_switch=can_switch_role(user)),
            )
            passenger_queue.try_promote(session)
            return
    ...
    user.driver_missed_offers = 0
    queue_service.mark_assigned(session, user)
    order_service.finalize_offer_message(session, order, "✅ Вы приняли заявку.")
    audit.record(session, "driver_accept", f"order={order.id} driver={user.id}")
    # V76: the passenger hears about the driver right after the accept press.
    order_service.notify_driver_found(session, order)
...
def driver_decline(session: Session, user: User, order_id: int, cat: str) -> None:
    ...
    * spam           — passenger gets a temporary order ban; driver unaffected.
    """
    # Serialize the final reason with accept and timeout. Old VK buttons must
    # never mutate an order that has already moved to another driver.
    order = (
        session.query(Order)
    ...
    if (
        ...
        return show_main_menu(session, user)
    timers.cancel("accept", order.id)
    order_service.finalize_offer_message(session, order, "🚫 Вы отказались от заявки.")
    queue_service.release_offer(session, user)
    order_service.clear_current_offer(session, order)
    # SessionLocal has autoflush disabled. Persist the released queue/order
    # locks before the FIFO query selects the next eligible driver.
    session.flush()
    on_line_kb = kb.driver_menu(on_line=True, show_role_switch=can_switch_role(user))

    # A passenger-selected request belongs only to the chosen driver. A
    # refusal never leaks it into the ordinary FIFO queue.
    if order.requested_driver_id:
        passenger = session.get(User, order.passenger_id)
        order.status = "cancelled"
        order.cancelled_at = time_utils.now()
        order.cancelled_by = "driver"
        order.last_decline_reason = cat
        timers.cancel_all_for_order(order.id)
        if cat == "away":
            queue_service.set_away(session, user)
            driver_keyboard = kb.driver_away_menu(can_switch_role(user))
            driver_text = "Вы отказались от персональной заявки и отлучились."
        else:
            driver_keyboard = on_line_kb
            driver_text = "Вы отказались от персональной заявки. Ваше место в очереди сохранено."
        set_state(session, user.vk_id, States.D_MENU)
        vk.send_message(user.vk_id, driver_text, keyboard=driver_keyboard)
        if passenger:
            reset(session, passenger.vk_id, States.MAIN_MENU)
            vk.send_message(
                passenger.vk_id,
                "Выбранный водитель отказался от заявки. Она не передавалась другим водителям.",
                keyboard=kb.direct_driver_unavailable_keyboard(),
            )
        audit.record(
            session,
            "direct_driver_decline",
            f"order={order.id} driver={user.id} reason={cat}",
        )
        passenger_queue.try_promote(session)
        return
    ...
    # --- Ordinary refusal reasons: keep position and pass to next driver --- #
    if cat == "dislike":
        vk.send_message(
            user.vk_id,
    ...
    vk.send_message(
        user.vk_id,
        f"Отказ от {label} принят. Заявка передана следующему водителю. "
        "Ваше место в очереди сохранено.",
        keyboard=on_line_kb,
    )
    set_state(session, user.vk_id, States.D_MENU)
    # Only after every eligible FIFO driver has declined does the standard
    # no-driver path publish a far/delivery request to the requests chat.
    order_service.offer_to_next_driver(session, order)
    passenger_queue.try_promote(session)
...
# A partner delivery asks when the driver reaches the SHOP, not the passenger.
PARTNER_DELIVERY_ETA_PROMPT = "Через сколько минут вы подъедете за заказом?"
...
def _show_eta_menu(session: Session, user: User, order: Order, intro: str | None = None) -> None:
    ...
    if delivery_service.is_partner_delivery(order):
        eta_text = PARTNER_DELIVERY_ETA_PROMPT
    else:
        eta_text = msg(session, "msg_eta_menu")
    text = f"{intro}\n\n{eta_text}" if intro else eta_text
    # Track the inline ETA prompt so choosing a preset/custom value can edit
    # this exact message and remove its buttons.
    if order.offer_outbox_id:
        from . import outbox_service
        outbox_service.cancel_or_delete(session, order.offer_outbox_id)
        order.offer_outbox_id = None
    # A voice request must remain playable after the driver accepts it. The
    # accepted-order/ETA card is a new tracked message, so attach the prepared
    # community-owned voice again instead of relying only on the old card.
    voice_attachment = vk.prepare_voice_attachment(user.vk_id, order.voice_attachment)
    if voice_attachment:
        order.voice_attachment = voice_attachment
...
def driver_eta_custom(session: Session, user: User) -> None:
    ...
    elif delivery_service.is_partner_delivery(order):
        vk.send_message(user.vk_id, "Введите цифрами, через сколько минут подъедете за заказом:")
    else:
...
def _apply_eta(session: Session, user: User, order: Order, eta: int) -> None:
    # Remove the time-choice buttons from the original message immediately.
    order_service.finalize_offer_message(
        session, order, f"✅ Выбрано время прибытия: {eta} мин."
    )
    ...
    is_partner_delivery = delivery_service.is_partner_delivery(order)
    ...
    if target:
        if _is_dispatcher_order(order):
            ...
            if is_partner_delivery:
                header = "\U0001F4E6 Вашу доставку взял водитель.\n"
                eta_line = f"\nВодитель заберёт заказ примерно через {eta} мин."
                question = ""
                # Актуальность по доставкам от партнёров не спрашиваем:
                # сообщение уйдёт без вопроса и без таймера подтверждения.
                order.actuality_confirmed = True
            else:
                header = ""
                eta_line = f"\nВодитель прибудет примерно через {eta} мин."
                question = "Вы ждёте машинку?"
            if order.actuality_confirmed:
                order.departure_prompt_outbox_id = None
                vk.send_message(
                    target.vk_id,
                    header + _driver_card(user) + eta_line,
                    keyboard=kb.passenger_ride_keyboard(),
                )
            else:
    ...
    if is_delivery:
        ...
        vk.send_message(
            user.vk_id,
            "Время подачи отправлено." if is_partner_delivery else "Время прибытия отправлено.",
...
def driver_set_eta(session: Session, user: User, text: str) -> None:
    ...
    if not digits:
        if delivery_service.is_delivery(order):
            ...
            vk.send_message(user.vk_id, "Введите целым числом минут, через сколько подъедете за заказом:")
        else:
            vk.send_message(user.vk_id, msg(session, "msg_eta_custom_prompt"))
        return
    ...
    if _is_dispatcher_order(order):
        ...
        if passenger:
            seated_text = (
                ...
                if delivery_service.is_partner_delivery(order)
                else "🚗 Поехали! Хорошей поездки."
            )
            vk.send_message(passenger.vk_id, seated_text, keyboard=kb.passenger_in_ride_keyboard())
...
def _driver_ride_kb(session: Session, order: Order):
    ...
    driver = session.get(User, order.driver_id) if order.driver_id else None
    # Партнёрская доставка: реквизиты водитель не отправляет.
    is_partner_delivery = delivery_service.is_partner_delivery(order)
    payment_details_enabled = bool(
        driver
        and driver.show_payment_details
        and _payment_details_ready(driver)
        and not is_partner_delivery
    )
    ...
    return kb.driver_ride_keyboard(
        ...
        eta_set=order.arrival_eta is not None,
        ...
        partner_delivery=is_partner_delivery,
    )
...
def driver_send_payment_details(session: Session, user: User) -> None:
    ...
    if delivery_service.is_partner_delivery(order):
        # По доставкам от партнёров реквизиты не отправляются.
        return vk.send_message(
            user.vk_id,
            "По доставкам от партнёров реквизиты не отправляются.",
            keyboard=_driver_ride_kb(session, order),
...
def driver_complete_delivery(session: Session, user: User, order: Order) -> None:
    ...
    if not _is_dispatcher_order(order):
        if client:
            order.passenger_rating_prompt_outbox_id = vk.send_tracked_message(
                client.vk_id,
    ...
    if has_parallel:
        ...
        # This sends the post-ride line chooser, which makes exactly one
        # transition to ``away``.  Calling set_away here as well scheduled the
        # same driver_away timer twice in the current transaction.
...
def driver_finish_prompt(session: Session, user: User) -> None:
    ...
    prompt = "\n\n".join(prompt_parts)
    # Keep the active-ride keyboard available below the fare prompt. Entering
    # a number still finishes the ride; buttons are routed as actions.
    vk.send_message(
        user.vk_id,
        prompt,
...
def driver_complete_ride(session: Session, user: User, text: str) -> None:
    ...
    if not _is_dispatcher_order(order):
        ...
        if passenger:
            total = price
            # Requisites are never attached to the final summary: the passenger
            # only gets them when the driver taps «Отправить реквизиты» during the
            # ride. If the button was not used, nothing has to be transferred.
            if order.payment_details_sent:
                payment_block = (
                ...
                payment_block = ""
            # По доставкам от партнёров итог по поездке не пишется.
            if not delivery_service.is_partner_delivery(order):
                vk.send_message(
                    passenger.vk_id,
                    "🧾 Итог по поездке #%d:\nИтого: %.0f ₽%s" % (order.id, total, payment_block),
                )
            rating_text, rating_file = bm.render(session, "ride_finished", total=f"{total:.0f}")
            order.passenger_rating_prompt_outbox_id = vk.send_tracked_message(
                passenger.vk_id,
                rating_text,
                keyboard=kb.rating_keyboard(order.id),
                attachment=rating_file or None,
            )
            reset(session, passenger.vk_id, States.MAIN_MENU)
...
def passenger_booking_start(session: Session, user: User) -> None:
    ...
    if existing:
        return passenger_show_booking(session, user)
    # Passenger and dispatcher use one booking product: long-distance trips.
    # There is no intermediate early/specific-time type selector.
    return passenger_booking_fill(session, user)


def passenger_booking_fill(session: Session, user: User) -> None:
    draft = {"type": "far_distance"}
    set_state(session, user.vk_id, States.P_BOOKING_DATE, {"booking": draft}, merge=False)
    vk.send_message(
        user.vk_id,
        "На какое число бронируем дальнюю поездку?",
        keyboard=kb.booking_date_keyboard(),
    )


def passenger_booking_type(session: Session, user: User, booking_type: str | None) -> None:
    """Compatibility for buttons sent before v35: always open long distance."""
    return passenger_booking_fill(session, user)
...
def _broadcast_booking_to_driver_chat(
    ...
    timers.schedule(
        "booking_chat", booking.id, booking_service.CHAT_UNCLAIMED_TIMEOUT_SECONDS,
        lambda: booking_service.expire_unclaimed_booking(booking.id),
    )
...
#  Dispatcher flow                                                             #
# --------------------------------------------------------------------------- #
_DISPATCHER_REPORT_STATES = (
    States.DISP_REPORT_INPUT,
)


def _handle_dispatcher_report(session, user, state, text, payload, attachments):
    """«Отправить отчет водителям» steps (dispatchers only).

    Returns True when the message was consumed by the report flow.
    """
    cmd = payload.get("cmd")
    if cmd == "disp_driver_report":
        disp_report_start(session, user)
        return True
    if cmd == "cancel_flow" and state in _DISPATCHER_REPORT_STATES:
        disp_report_cancel(session, user)
        return True
    if cmd:
        return False
    if state == States.DISP_REPORT_INPUT:
        disp_report_submit(session, user, text, attachments)
        return True
    return False
...
def handle_dispatcher(session, user, state, text, payload, attachments):
    ...
    # «Отправить отчет водителям»: one message containing text and a photo.
    if state in _DISPATCHER_REPORT_STATES or cmd == "disp_driver_report":
        if _handle_dispatcher_report(session, user, state, text, payload, attachments):
            return
    ...
    if cmd == "queue":
        return show_queue(session, user, dispatcher_view=True)
    if cmd == "price":
        return show_price(session, user)
    if cmd == "price_section":
        return show_price_section(session, user, payload.get("key"))
...
def disp_create_order_from_draft(session: Session, user: User) -> None:
    ...
    selection = data.get("extras", [])
    order_type = (
        draft.get("order_type")
        if draft.get("order_type") in ("delivery", delivery_service.PARTNER_DELIVERY_TYPE)
        else "regular"
    )
    order = Order(
        ...
        night_surcharge=(order_type == "regular" and night_tariff.is_night(session)),
        customer_name=draft.get("customer_name"),
        customer_phone=draft.get("customer_phone"),
        # A freshly created dispatcher order has no refusal history and has
        # not entered a route fallback stage.
        last_decline_reason=None,
        parallel_route_fallback=False,
    )
    session.add(order)
    session.flush()
    timers.schedule(
...
def disp_cancel_order(session: Session, user: User, order_id) -> None:
    ...
    driver = session.get(User, order.driver_id) if order.driver_id else None
    from . import parallel_orders
    # Close one-minute route cards tied to the trip before freeing its driver.
    parallel_orders.release_route_offers_for_trip(session, order)
    order.status = "cancelled"
    order.cancelled_at = time_utils.now()
    order.cancelled_by = "dispatcher"
    ...
    if driver:
        vk.send_message(driver.vk_id, f"Диспетчер отменил заявку #{order.id}. Ложный вызов не начисляется.")
        if parallel_orders.has_pending(session, driver):
            parallel_orders.promote_after_current(session, driver)
        else:
            queue_service.restore_position(session, driver)
            reset(session, driver.vk_id, States.D_MENU)
...
def disp_report_start(session: Session, user: User) -> None:
    """Ask a dispatcher for one message containing both text and a photo."""
    vk.send_message(
        user.vk_id,
        "📤 Отчет водителям.\n\n"
        "Отправьте одним сообщением:\n"
        "• фотографию;\n"
        "• текст отчета с указанием реквизитов.\n\n"
        "После отправки отчет получат все пользователи с ролью водителя.",
        keyboard=kb.cancel_keyboard(),
    )
    set_state(session, user.vk_id, States.DISP_REPORT_INPUT, {}, merge=False)


def disp_report_submit(
    session: Session,
    user: User,
    text: str,
    attachments,
) -> None:
    """Validate and immediately broadcast a dispatcher report to drivers."""
    if not user.has_role(ROLE_DISPATCHER):
        set_state(session, user.vk_id, States.DISP_MENU, {}, merge=False)
    ...
    if not text:
        return vk.send_message(
            user.vk_id,
            "Текст не найден. Отправьте фотографию и текст с реквизитами одним сообщением.",
            keyboard=kb.cancel_keyboard(),
        )

    # Only a VK photo is accepted. Documents, videos and other attachments
    # must never be forwarded by this dispatcher-only flow.
    photos = [
        attachment
    ...
    if not photos:
        return vk.send_message(
            user.vk_id,
            "Фотография не найдена. Отправьте фотографию и текст с реквизитами одним сообщением.",
            keyboard=kb.cancel_keyboard(),
        )

    uploaded = vk.reupload_attachments(user.vk_id, photos[:1])
    if not uploaded:
        return vk.send_message(
            user.vk_id,
            "Не удалось обработать фотографию. Отправьте сообщение ещё раз.",
            keyboard=kb.cancel_keyboard(),
        )
    attachment = ",".join(uploaded)
    set_state(session, user.vk_id, States.DISP_MENU, {}, merge=False)
    # target="driver" reaches every user with the driver role — and only them.
    broadcast_service.start(user.vk_id, text, attachment, "driver")
    vk.send_message(
        user.vk_id,
        "✅ Отчет отправляется всем водителям. Ожидайте отчёт о доставке.",
        keyboard=kb.dispatcher_menu(can_switch_role(user)),
    )


def disp_report_cancel(session: Session, user: User) -> None:
    set_state(session, user.vk_id, States.DISP_MENU, {}, merge=False)
    vk.send_message(
        user.vk_id,
...
def _passenger_labels(session: Session) -> dict:
    ...
    return {
        ...
        "btn_booking": button_label(session, "btn_booking", "📅 Бронь дальние поездки"),
...
def driver_return_to_active(session: Session, user: User) -> None:
    """Restore the current trip and its keyboard from any auxiliary screen."""
    order = active_order_for(session, user, as_driver=True)
    if not order:
        return show_main_menu(session, user)
    waiting_for_price = get_state(session, user.vk_id).state == States.D_FINISH_PRICE
    if waiting_for_price:
        ...
        text = (
            ...
            "Введите итоговую стоимость поездки одним числом (₽):"
        )
    else:
        set_state(
            session,
        ...
        text = f"Возвращаемся к активной заявке #{order.id}."
    vk.send_message(
        user.vk_id,
        text,
        keyboard=_active_driver_order_keyboard(session, order),
...
def passenger_update_address(session: Session, user: User, text: str) -> None:
    ...
    order.parallel_route_fallback = False
    order.parallel_auto_excluded_driver_ids = None
...
def driver_cancel_active(session: Session, user: User, reason: str) -> None:
    ...
    # active_order_for may repair one legacy promoted row in memory. Persist
    # that repair first, then lock/reload the exact current trip so completion,
    # passenger cancellation and admin cancellation cannot race this action.
    session.flush()
    order = (
        session.query(Order)
    ...
    if (
        ...
        or order.status not in ("searching", "assigned", "arrived", "in_progress")
    ):
        return show_main_menu(session, user)
    reason = (reason or "").strip().casefold()
    if reason not in ("no_show", "customer_cancel", "car"):
        log.error(
    ...
    timers.cancel_all_for_order(order.id)
    from . import parallel_orders
    # Any automatic parallel card was valid only while this exact trip was
    # active. Close it now instead of waiting for its 60-second callback.
    parallel_orders.release_route_offers_for_trip(session, order)
    passenger = session.get(User, order.passenger_id)
    if reason == "customer_cancel":
        # The two-minute window starts only after the driver has supplied ETA:
        # _apply_eta records that moment in driver_departed_at.  Reuse the
        # passenger cancellation setting so both cancellation paths share the
        # exact same strict boundary (> grace creates a false call; <= does not).
        past_grace = bool(
            order.driver_departed_at
            and not _passenger_within_grace(session, order)
        )
        order.status = "cancelled"
        order.cancelled_at = time_utils.now()
        ...
        if passenger and not _is_dispatcher_order(order):
            if past_grace:
                fake_calls_service.create(session, order, user)
                fake_call_created = True
            else:
                reset(session, passenger.vk_id, States.MAIN_MENU)
                vk.send_message(
                    passenger.vk_id,
                    "Ваша заявка отменена по вашей просьбе.",
                    keyboard=kb.passenger_menu(
                        can_switch_role(passenger),
                        _passenger_labels(session),
                    ),
                )
        elif passenger and _is_dispatcher_order(order):
            vk.send_message(
                passenger.vk_id,
                f"❌ Водитель отменил заявку #{order.id}.\n"
                "Причина: клиент захотел отменить. Ложный вызов не начисляется.\n"
                f"{order_service.dispatcher_driver_details(user)}",
            )
        has_parallel = parallel_orders.has_pending(session, user)
        if not has_parallel:
        ...
        _leave_temporary_chat_line(session, user, order)
        reset(session, user.vk_id, States.D_MENU)
        result_text = f"✅ Заявка #{order.id} отменена: клиент захотел отменить."
        if _is_dispatcher_order(order):
            result_text += " Диспетчерской заявке ложный вызов не начисляется."
        elif fake_call_created:
            result_text += " Прошло более 2 минут после указания времени прибытия — ложный вызов создан."
        else:
            result_text += " Прошло не более 2 минут после указания времени прибытия — ложный вызов не создан."
        vk.send_message(user.vk_id, result_text)
        audit.record(
            session,
            "driver_customer_cancel",
            f"order={order.id} driver={user.id} fake_call={int(fake_call_created)}",
        )
        if has_parallel:
            parallel_orders.promote_after_current(session, user)
        else:
...
# Statuses in which a request can no longer be claimed from the requests chat.
CHAT_CLAIM_CLOSED_STATUSES = (
    "assigned", "arrived", "in_progress", "parallel_assigned", "completed", "cancelled",
)


def driver_take_from_chat(session: Session, user: User, order_id) -> None:
    """Assign a request published in the requests chat to this driver.

    The claim is refused only when the driver already has an active ride or
    when somebody else owns the request. Away status, an empty line, a dead
    offer lock and missing car details are repaired here instead of dropping
    the claim, so a pressed chat button always has a visible result.
    """
    order = (
        session.query(Order).filter(Order.id == int(order_id)).with_for_update().one_or_none()
        if order_id
        else None
    )
    if order is None:
        _chat_claim_reply(session, user, f"заявка №{order_id} не найдена.")
        log.warning("Chat claim: order %s not found (driver=%s)", order_id, user.id)
        return vk.send_message(user.vk_id, "Заявка уже недоступна.")
    active = active_order_for(session, user, as_driver=True)
    if active and active.id != order.id:
        _chat_claim_reply(
            session, user,
            f"вы уже выполняете заявку №{active.id} — эта осталась свободной.",
        )
        log.warning("Chat claim rejected: driver=%s busy with order=%s", user.id, active.id)
        return vk.send_message(
            user.vk_id,
            "Вы уже выполняете заявку. Новую можно взять только через раздел «Параллельные заявки».",
        )
    if order.driver_id and order.driver_id != user.id:
        holder = session.get(User, order.driver_id)
        holder_name = (holder.full_name if holder else None) or "другой водитель"
        _chat_claim_reply(session, user, f"заявку №{order.id} уже взял водитель {holder_name}.")
        log.warning("Chat claim rejected: order=%s owned by driver=%s", order.id, order.driver_id)
        return vk.send_message(user.vk_id, "Заявку уже взял другой водитель.")
    if order.status in CHAT_CLAIM_CLOSED_STATUSES:
        _chat_claim_reply(session, user, f"заявка №{order.id} уже закрыта.")
        log.warning("Chat claim rejected: order=%s status=%s", order.id, order.status)
        return vk.send_message(user.vk_id, "Заявка уже недоступна.")
    # A live offer must not block the chat: it is handed to the next driver
    # right after this request is locked onto the current one.
    pending_offer = offered_order_for(session, user)
    if pending_offer is not None and pending_offer.id == order.id:
        pending_offer = None
    if pending_offer is not None:
        timers.cancel("accept", pending_offer.id)
        pending_offer.offered_driver_id = None
        queue_service.release_offer(session, user)
        reset(session, user.vk_id, States.D_MENU)
        session.flush()
    # Incomplete car details are only a reminder now: they must never cancel a
    # claim and leave the request hanging in the chat.
    if not lines.has_complete_car(user):
        vk.send_message(
            user.vk_id,
            "Заявка закреплена за вами. Пожалуйста, заполните «Моя машина»: марка, цвет и госномер.",
        )
        log.warning("Chat claim with incomplete car order=%s driver=%s", order.id, user.id)
    was_offline = not bool(user.is_on_line)
    if was_offline or user.driver_status == "away":
        queue_service.join_queue(session, user, lines.city_id_by_name(session, user.current_line))
        user.is_on_line = True
    timers.cancel("driver_chat", order.id)
    timers.cancel("dispatcher_unclaimed", order.id)
    ...
    user.driver_missed_offers = 0
    queue_service.mark_assigned(session, user)
    # Persist the assignment before any VK work: a failed edit or notification
    # must never roll the claim back.
    session.flush()
    if pending_offer is not None:
        log.warning(
            "Offer %s released: driver %s claimed chat order %s",
            pending_offer.id, user.id, order.id,
        )
        vk.send_message(
            user.vk_id,
            f"Заявка #{pending_offer.id} передана следующему водителю: вы взяли заявку №{order.id} из чата.",
        )
        try:
            order_service.offer_to_next_driver(session, pending_offer)
        except Exception as exc:  # noqa: BLE001
            log.exception("Could not re-offer order %s: %s", pending_offer.id, exc)
    try:
        order_service.finalize_chat_order_notice(session, order, user)
    except Exception as exc:  # noqa: BLE001
        log.exception("Chat notice finalize failed for order %s: %s", order.id, exc)
    ...
    if order_service.driver_chat_reason_label(session, order):
        ...
        return
    if passenger:
        order.chat_actuality_prompt_outbox_id = vk.send_tracked_message(
            passenger.vk_id,
            f"Водитель нашёлся. Ваша заявка актуальна?\n{order_service.order_text(order)}",
            keyboard=kb.chat_order_actual_keyboard(order.id),
        )
        timers.schedule("chat_actual", order.id, 60, lambda: chat_actuality_timeout(order.id))
...
def passenger_chat_order_actual(session: Session, user: User, order_id, actual: bool) -> None:
    ...
    timers.cancel("chat_actual", order.id)
    if order.chat_actuality_prompt_outbox_id:
        from . import outbox_service
        outbox_service.finalize_tracked_message(
            session,
            order.chat_actuality_prompt_outbox_id,
            "✅ Вы ответили: заявка актуальна." if actual else "❌ Вы ответили: заявка неактуальна.",
        )
        order.chat_actuality_prompt_outbox_id = None
    driver = session.get(User, order.driver_id) if order.driver_id else None
    if not actual:
        order.status = "cancelled"
        order.cancelled_at = time_utils.now()
...
def chat_actuality_timeout(order_id: int) -> None:
    """Cancel a chat-claimed request when the passenger is silent for 1 minute."""
    from common.database import session_scope
    with session_scope() as session:
        order = session.query(Order).filter(Order.id == int(order_id)).with_for_update().one_or_none()
        if not order or order.status != "assigned" or not order.chat_actuality_prompt_outbox_id:
            return
        from . import outbox_service
        outbox_service.cancel_or_delete(session, order.chat_actuality_prompt_outbox_id)
        order.chat_actuality_prompt_outbox_id = None
        order.status = "cancelled"
        order.cancelled_at = time_utils.now()
        order.cancelled_by = "system"
        ...
        if driver:
            ...
            vk.send_message(
                driver.vk_id,
                "Пассажир не подтвердил актуальность заявки за 5 минут. Заявка отменена.",
                keyboard=kb.driver_menu(bool(driver.is_on_line), can_switch_role(driver)),
            )
        passenger_queue.try_promote(session)
