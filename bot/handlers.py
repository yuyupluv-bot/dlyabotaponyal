...
def create_passenger_order(session: Session, user: User, order_type: str) -> None:
    ...
    selection = data.get("extras", []) if normalized_type == "regular" else []
    order = Order(
...
def driver_payment_start(session: Session, user: User, dispatcher: bool = False) -> None:
    ...
    set_state(
        ...
        user.vk_id,
        States.D_PAYMENT_TYPE,
        {"payment_draft": ({"dispatcher": True} if dispatcher else {})},
        merge=False,
    )


def driver_payment_method(session: Session, user: User, payment_type: str | None) -> None:
    previous = dict(get_data(session, user.vk_id).get("payment_draft") or {})
    if payment_type not in ("phone", "card"):
        return driver_payment_start(
            session, user, dispatcher=bool(previous.get("dispatcher"))
        )
    draft = {"type": payment_type}
    if previous.get("dispatcher"):
        draft["dispatcher"] = True
...
def driver_payment_recipient(session: Session, user: User, text: str) -> None:
    ...
    if draft.get("dispatcher"):
        # Dispatcher requisites are used by the Sunday debt notices, so the
        # in-ride requisites switch of drivers is irrelevant here.
        vk.send_message(
            user.vk_id,
            "Реквизиты сохранены: "
            + _payment_details_text(user)
            + "\nИх увидят водители в воскресном уведомлении о долге.",
        )
...
def driver_payment_cancel(session: Session, user: User) -> None:
    ...
    if (get_data(session, user.vk_id).get("payment_draft") or {}).get("dispatcher"):
        vk.send_message(
            user.vk_id,
...
def _handle_payment_wizard(session, user, state, text, payload):
    """Shared requisites wizard steps (drivers and dispatchers).

    Returns True when the message was consumed by the wizard.
    """
    cmd = payload.get("cmd")
    if cmd == "cancel_flow" and state in _PAYMENT_STATES:
    ...
    if cmd == "payment_method":
        ...
        return True
    if cmd == "payment_recipient_skip":
        driver_payment_recipient(session, user, "")
        return True
    if cmd:
        return False
    if state == States.D_PAYMENT_PHONE:
    ...
    if state == States.D_PAYMENT_RECIPIENT:
        ...
        return True
    if state == States.D_PAYMENT_TYPE:
        driver_payment_start(session, user, dispatcher=True)
        return True
    return False
...
def handle_dispatcher(session, user, state, text, payload, attachments):
    ...
    # Requisites wizard: without this the dispatcher was dropped back into the
    # main menu on the very first step.
    if state in _PAYMENT_STATES or cmd in (
        "payment_method",
        "payment_recipient_skip",
    ):
        if _handle_payment_wizard(session, user, state, text, payload):
            return
    ...
    if cmd == "disp_income":
        return disp_show_income(session, user)
    if cmd == "disp_payment":
        return disp_payment_details(session, user)
    if cmd == "disp_orders":
...
def disp_payment_details(session: Session, user: User) -> None:
    """Dispatcher requisites: the very same wizard the drivers use."""
    if _payment_details_ready(user):
        vk.send_message(
            user.vk_id,
    ...
    return driver_payment_start(session, user, dispatcher=True)


def disp_show_income(session: Session, user: User) -> None:
    """«My details»: unpaid driver debts plus the saved requisites."""
    from . import dispatcher_report_service

    text = dispatcher_report_service.dispatcher_income_text(session, user.id)
    if _payment_details_ready(user):
        text += "\n\n" + "💳 Ваши реквизиты: " + _payment_details_text(user)
    else:
        text += (
            "\n\n💳 Реквизиты не заполнены. "
            "Водители не увидят, куда переводить деньги."
        )
    set_state(session, user.vk_id, States.DISP_MENU, {}, merge=False)
    vk.send_message(user.vk_id, text, keyboard=kb.dispatcher_info_menu())
