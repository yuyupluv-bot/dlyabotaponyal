...
class VKClient:
    ...
    def edit_message(
        ...
        The original attachment must be repeated on every edit: VK drops
        voice messages and photos from a message edited without it.
        """
    ...
    def reupload_attachments(self, peer_id: int, attachments: list[dict]) -> list[str]:
        ...
        for att in attachments:
            try:
                ...
                if att_type == "photo":
                    result.append(self._forward_photo(peer_id, att["photo"]))
    ...
    @staticmethod
    def photo_source_url(photo: dict) -> str | None:
        """Best available image URL for a VK photo object.

        Long Poll may deliver photos without ``sizes`` (older ``photo_*``
        fields or ``orig_photo`` only). Returning None instead of raising keeps
        a text-free photo message forwardable through the fallback paths.
        """
        if not isinstance(photo, dict):
            return None
        sizes = photo.get("sizes")
        if isinstance(sizes, list) and sizes:
            usable = [s for s in sizes if isinstance(s, dict) and s.get("url")]
            if usable:
                usable.sort(key=lambda s: (s.get("width") or 0) * (s.get("height") or 0))
                return usable[-1]["url"]
        ...
        for key in (
            ...
            "photo_604", "photo_130", "photo_75", "url",
        ):
            value = photo.get(key)
            if value:
                return value
    ...
    @staticmethod
    def photo_reference(photo: dict) -> str | None:
        """Direct ``photo<owner>_<id>[_<access_key>]`` reference, if available."""
        if not isinstance(photo, dict):
            return None
        ...
        if owner is None or obj_id is None:
            return None
        ref = f"photo{owner}_{obj_id}"
        access = photo.get("access_key")
        if access:
            ref += f"_{access}"
        return ref

    def _forward_photo(self, peer_id: int, photo: dict) -> str:
        """Forward an incoming photo even when it arrived without any text.

        A fresh community-owned copy is preferred. When VK refuses the upload
        (for example ``photo is undefined`` after an expired CDN link), fall
        back to the original reference so the photo still reaches the peer.
        """
        try:
            return self._reupload_photo(peer_id, photo)
        except Exception as exc:  # noqa: BLE001
            reference = self.photo_reference(photo)
            if reference:
                log.warning(
                    "Photo re-upload failed (%s); forwarding original reference %s",
                    exc,
                    reference,
                )
                return reference
    ...
    def _reupload_photo(self, peer_id: int, photo: dict) -> str:
        url = self.photo_source_url(photo)
        if not url:
            raise ValueError("photo source URL is missing")
        content = b""
        ...
        for attempt in range(3):
            try:
                response = requests.get(
                    url,
                    timeout=30,
                    headers={"User-Agent": "Mozilla/5.0 (compatible; vk-taxi-bot)"},
                )
                response.raise_for_status()
        ...
        if not content:
            raise ValueError(f"photo download failed: {last_error}")
        ...
        try:
            ...
            for attempt in range(2):
                try:
                    with self._api_lock:
                        uploaded = self.uploader.photo_messages(photos=tmp, peer_id=peer_id)
                    if uploaded:
                        saved = uploaded[0]
                        break
                except Exception as exc:  # noqa: BLE001
            ...
            if not isinstance(saved, dict) or saved.get("owner_id") is None or saved.get("id") is None:
                raise ValueError(f"VK did not return the saved photo: {last_error}")
            ref = f"photo{saved['owner_id']}_{saved['id']}"
            if saved.get("access_key"):
                ref += f"_{saved['access_key']}"
            return ref
        finally:
            try:
                os.remove(tmp)
