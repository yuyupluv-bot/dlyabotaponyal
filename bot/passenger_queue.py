...
def dispatch_new_order(session: Session, order: Order) -> None:
    ...
    """
    # Return-route priority is: a free driver on the pickup line first; then any
    # other eligible free driver; only when no free drivers exist may a driver
    # already travelling there receive a parallel offer. This applies to explicit «из Пашии/Кусьи» requests and when
    ...
    for entry in waiting:
        ...
        if route_city or free_city:
            if order.last_decline_reason == parallel_orders.ROUTE_FALLBACK_REASON:
                # A declined/expired route-parallel order is now ordinary work:
                # offer it to the matching Пашия/Кусья line or Горнозаводск.
                line_scope = "normal"
                line_name = free_city or route_city or "Горнозаводск"
                has_driver = queue_service.has_waiting_driver(
                    session, line_name, line_scope="normal"
                )
                ...
                # Any eligible free driver always has priority over a busy
                # destination driver and a parallel reservation.
                has_driver = order_service.has_eligible_waiting_driver(session, order)
