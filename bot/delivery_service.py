...
PARTNER_DELIVERY_TYPE = "partner_delivery"

# The word «доставка» in its usual inflected forms.
_DELIVERY_WORD = re.compile(
    r"(?<![а-яё])достав(?:к[а-яё]*|ить|ьте|ь|им|лю|ляю|ляет|ляют)(?![а-яё])",
    re.IGNORECASE,
)

# Partner shops. Only the shop NAME is matched here: the word «доставка» is
# checked separately, so «от красного бархата до Ленина 5» stays an ordinary
# taxi ride and is never turned into a delivery.
_PARTNER_PATTERNS = tuple(re.compile(pattern, re.IGNORECASE) for pattern in (
    # Красный бархат: падежи, дефис, слитно, опечатки (бархот, барх, барш).
    r"(?<![а-яё])красн\w*\s*[-–—]?\s*бар[хш]\w*",
    r"(?<![а-яё])кр\.?\s*бар[хш]\w*",
    r"(?<![а-яё])бар[хш]ат\w*",
    r"(?<![а-яё])бар[хш]от\w*",
    r"(?<![a-z])krasn\w*\s*barh?\w*",
    r"(?<![a-z])barh?at\w*",
    # Галактика: падежи и частые опечатки (галлактика, галактка, галактика24).
    r"(?<![а-яё])гал{1,2}акт\w*",
    r"(?<![а-яё])гал\.?\s*актик\w*",
    r"(?<![a-z])gala[kc]ti[kc]\w*",
    r"(?<![a-z])galaxy(?![a-z])",
))
...
def has_delivery_word(text: str | None) -> bool:
    """True when the request literally mentions «доставка» in any form."""
    return bool(_DELIVERY_WORD.search(_normalize(text)))


def is_partner_text(text: str | None) -> bool:
    """True when the request mentions a partner shop, in any spelling."""
    normalized = _normalize(text)
    return any(pattern.search(normalized) for pattern in _PARTNER_PATTERNS)


def is_partner_delivery_text(text: str | None) -> bool:
    """Partner delivery = shop name AND the word «доставка» together."""
    return is_partner_text(text) and has_delivery_word(text)


def is_delivery_text(text: str | None) -> bool:
    """Ordinary delivery: the word «доставка» without a partner shop.

    A partner name alone («от галактики до Ленина 5») is a taxi ride.
    """
    return has_delivery_word(text)


def _request_text(order: Order) -> str:
    return " ".join(
        value for value in (order.route_text, order.address_from, order.address_to)
        if value
    )


def is_partner_delivery(order: Order) -> bool:
    """Partner delivery follows the ORDINARY ride flow, not the price flow."""
    if order.order_type == PARTNER_DELIVERY_TYPE:
        return True
    if is_partner_delivery_text(_request_text(order)):
        # Repair the row inside the current transaction so every later stage
        # also treats it as a partner delivery.
        order.order_type = PARTNER_DELIVERY_TYPE
        return True
    return False


def is_delivery(order: Order) -> bool:
    """Delivery where the DRIVER names the price.

    Partner deliveries are excluded on purpose: the driver is never asked for
    a delivery price there, the sum is entered at the end like a normal ride.
    """
    if is_partner_delivery(order):
        return False
    if order.order_type == "delivery":
        return True
    if is_delivery_text(_request_text(order)):
        # Repair the row in the current transaction so every later stage also
        # follows the delivery flow and asks the driver for a price.
...
def submit_price(session: Session, driver: User, text: str) -> None:
    ...
    if not _price_in_range(price):
        ...
        return
    order.price = price
    # Reuse the durable passenger-confirmation flag for the delivery offer.
    # It lets the 120-second callback distinguish an answered offer even when
    # the button press and timer were already running at the same instant.
    order.actuality_confirmed = False
    ...
    if passenger:
        vk.send_message(
            passenger.vk_id,
            msg(session, "msg_delivery_offer", price=price, amount=price),
    ...
    timeout = get_int(session, "delivery_confirm_timeout", 120)
    oid = order.id
...
def passenger_response(session: Session, passenger: User, order_id: int, agree: bool) -> None:
    ...
    if not order or order.passenger_id != passenger.id:
        return
    if order.status not in ("assigned", "searching") or order.actuality_confirmed:
        return
...
def _confirm(session: Session, order: Order, passenger: User | None, driver: User | None) -> None:
    ...
    if driver:
        # Track this inline ETA prompt; _apply_eta removes the buttons after
        # the driver chooses a delivery time.
...
def _confirm_timeout(order_id: int) -> None:
    ...
    with session_scope() as session:
        ...
        if (
            not order
            or order.status not in ("assigned", "searching")
            or order.actuality_confirmed
        ):
            return
