...
from common.models import AdminLog, BlockedUser, User
...
def _run(admin_vk_id: int, text: str, attachment: str | None, target: str) -> None:
    ...
    try:
        with session_scope() as db:
            # Administrative blocks exist both on users and in blocked_users.
            # Exclude both before splitting recipients into audiences.
            blocked_vk_ids = {vk_id for (vk_id,) in db.query(BlockedUser.vk_id).all()}
            users = db.query(User).filter(
                User.vk_id.isnot(None),
                User.is_blocked.is_(False),
            ).all()
            users = [u for u in users if u.vk_id not in blocked_vk_ids]
            if target == "driver":
                recipients = [u.vk_id for u in users if u.has_role("driver")]
            elif target == "passenger":
                # Every account technically keeps the passenger role, including
                # drivers/admins. Passenger broadcast means passenger-only users.
                recipients = [
                    u.vk_id for u in users
                    if set(u.roles_list()) == {"passenger"}
                ]
            else:
