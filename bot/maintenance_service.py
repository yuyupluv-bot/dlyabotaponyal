...
_BATCH_SIZE = 10_000
# Business data (users, orders, reviews, bookings, fake calls, commissions,
# admin logs) is never touched here: only technical bookkeeping expires.
FINISHED_JOB_RETENTION_DAYS = 30
LOGIN_ATTEMPT_RETENTION_DAYS = 180


def _delete_id_batch(session, model, condition, *extra_conditions) -> int:
    ids = [
        row_id for (row_id,) in session.query(model.id)
        .filter(condition, *extra_conditions)
...
def cleanup_once() -> tuple[int, int]:
    ...
    with session_scope() as session:
        # Weekly report markers are business bookkeeping: deleting them made
        # the Sunday report reappear every single day.
        events = _delete_id_batch(
            session,
            ProcessedEvent,
            ProcessedEvent.created_at < event_cutoff,
            ~ProcessedEvent.event_key.like("weekly_%"),
        )
        messages = 0
        ...
        _delete_id_batch(
            ...
            and_(
                ...
                ScheduledJob.run_at
                < now - dt.timedelta(days=FINISHED_JOB_RETENTION_DAYS),
            ),
        )
        _delete_id_batch(
            session,
            ...
            < now - dt.timedelta(days=LOGIN_ATTEMPT_RETENTION_DAYS),
        )
