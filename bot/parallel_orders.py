...
def notify_busy_drivers(session: Session, order: Order) -> None:
    """Notify busy drivers only after all free-driver options are exhausted."""
    if _free_driver_has_priority(session, order):
        return
...
def decline_route_offer(session: Session, driver: User, order_id: int) -> None:
    """Decline a route offer and immediately try ordinary free drivers."""
    order = _parallel_candidate_filter(
...
def _fallback_to_free_drivers(session: Session, order: Order, driver: User | None = None) -> None:
    """After a parallel decline, immediately hand the order to a free driver.

    Prefer a free driver on the request's Пашия/Кусья line, then use the normal
    matching-line + Горнозаводск fallback. If nobody is free, keep the order in
    the passenger queue so a driver becoming free later receives it.
    """
    ...
    pickup_line = free_line_city(order) or route_priority_city(order)
    line_scope = "normal"
    if pickup_line and queue_service.has_waiting_driver(
        session, pickup_line, line_scope="exact"
    ):
        line_scope = "exact"
    elif not queue_service.has_waiting_driver(
        session, pickup_line or "Горнозаводск", line_scope="normal"
    ):
    ...
    order_service.offer_to_next_driver(
        ...
        line_name=pickup_line or "Горнозаводск",
    )
...
def _route_offer_timeout(order_id: int) -> None:
    ...
    with session_scope() as session:
        ...
        if (not order
            ...
            return
        # The route offer expired: stop reserving it for busy drivers and hand
        # it to an ordinary free driver immediately. The follow-up promotion
        # also keeps the rest of the passenger queue moving.
