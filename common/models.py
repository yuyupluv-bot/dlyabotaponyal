...
class DriverQueue(Base):
    ...
    # Outbox id of the first-in-queue notice, so it can be deleted as soon
    # as the driver stops being the front of the line.
    front_notice_outbox_id = Column(Integer)
...
class DriverSelection(Base):
    """Short exclusive hold while a passenger types a request for one driver."""

    __tablename__ = "driver_selections"
...
class Order(Base):
    ...
    # Explicit passenger choice. These requests never enter the FIFO queue.
    requested_driver_id = Column(Integer, ForeignKey("users.id"), nullable=True, index=True)
    ...
    # Durable state of the single automatic route-parallel offer.  The offer
    # belongs both to this waiting order and to one concrete active trip, so a
    # stale button cannot be accepted after that trip has ended or changed.
    parallel_offer_driver_id = Column(Integer, ForeignKey("users.id"), nullable=True, index=True)
    parallel_offer_trip_id = Column(Integer, ForeignKey("orders.id"), nullable=True, index=True)
    parallel_offer_outbox_id = Column(Integer, nullable=True)
    # Drivers who explicitly declined or timed out this exact automatic offer.
    # This is deliberately separate from ordinary FIFO decline history: manual
    # parallel claiming must remain available.
    parallel_auto_excluded_driver_ids = Column(Text)
    # Durable route stage. Unlike last_decline_reason this is not overwritten
    # by a later ordinary Gornozavodsk driver decline.
    parallel_route_fallback = Column(Boolean, default=False, nullable=False)
    # This flag is read only while this row is the driver's active trip.  It is
    # therefore reset naturally when the next trip starts and never becomes a
    # permanent profile-level restriction.
    parallel_auto_offers_disabled = Column(Boolean, default=False, nullable=False)
    ...
    passenger_rating_prompt_outbox_id = Column(Integer, nullable=True)
    driver_rating_prompt_outbox_id = Column(Integer, nullable=True)
    chat_actuality_prompt_outbox_id = Column(Integer, nullable=True)
    ...
    # Passenger notices about the driver who currently holds the offer.
    # They are deleted as soon as the request moves to another driver.
    offer_notice_outbox_id = Column(Integer, nullable=True)
    search_notice_outbox_id = Column(Integer, nullable=True)
...
class PassengerQueue(Base):
    ...
    actuality_prompt_outbox_id = Column(Integer, nullable=True)
    created_at = Column(DateTime(timezone=True), default=_now, nullable=False, index=True)
