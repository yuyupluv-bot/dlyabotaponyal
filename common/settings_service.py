...
DEFAULTS: dict[str, str] = {
    ...
    "btn_booking": "📅 Бронь дальние поездки",
    "btn_rules": "📜 Правила",
    ...
    # Admin-editable timeout defaults. Existing database values are preserved.
    "driver_accept_timeout": "60",
    "driver_chat_timeout": str(config.DRIVER_CHAT_TIMEOUT),
    "driver_chat_far_timeout": "2700",
    "driver_chat_delivery_timeout": "2700",
    "booking_chat_timeout": "2700",
    "driver_chat_peer_id": str(config.DRIVER_CHAT_PEER_ID),
    "driver_fallback_chat_peer_id": str(config.DRIVER_FALLBACK_CHAT_PEER_ID),
    "msg_offer_timeout_driver": "⏰ Вы не приняли заявку вовремя. Вы перемещены в конец очереди, но остаётесь на линии.",
...
def msg(session: Session, key: str, **fmt: object) -> str:
    ...
    if fmt:
        try:
            ...
            # Never expose a broken admin/legacy placeholder to passengers.
            # Format the known-good built-in template instead.
            fallback = DEFAULTS.get(key, "")
            try:
                return fallback.format(**fmt) if fallback else ""
            except (KeyError, IndexError, ValueError):
                return fallback
    return template
