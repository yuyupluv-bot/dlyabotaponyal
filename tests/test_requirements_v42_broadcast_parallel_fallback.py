...
class BroadcastAndParallelV42Tests(unittest.TestCase):
    ...
    def test_parallel_decline_dispatches_to_free_drivers(self):
        ...
        self.assertIn("_fallback_to_free_drivers(", route_decline)
        self.assertIn("offer_already_closed=True", route_decline)
    ...
    def test_queued_fallback_reaches_later_free_driver(self):
        ...
        branch = source.split("if parallel_orders.is_route_fallback(order):", 1)[1].split("elif route_city", 1)[0]
        self.assertIn('line_scope = "exact"', branch)
        self.assertIn('line_name = "Горнозаводск"', branch)
