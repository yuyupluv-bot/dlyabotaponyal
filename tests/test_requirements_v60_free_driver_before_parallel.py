...
class FreeDriverBeforeParallelV60Tests(unittest.TestCase):
    ...
    def test_new_route_order_checks_free_driver_before_parallel(self):
        ...
        free=dispatch.index('session, route_city, line_scope="exact"')
        parallel=dispatch.index("if parallel_orders.notify_busy_drivers(session, order):")
    ...
    def test_recovery_no_longer_suppresses_free_driver_for_route_parallel(self):
        ...
        self.assertIn("only the exact village line", promote)
        self.assertIn('line_name = "Горнозаводск"', promote)
